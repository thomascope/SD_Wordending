...
function make_WM_VBM_explicit_mask(scan_paths, path_to_template_6, prefix)
...
split_stem = regexp(scan_paths, '/structural/', 'split');
...
for i = 1:length(scan_paths)
    oldfilenames{i}= cellstr([split_stem{i}{1} '/structural/wc2' split_stem{i}{2}]);
    newfilenames{i}= cellstr([split_stem{i}{1} '/structural/s_0_wc2' split_stem{i}{2}]);
    try
...
newfilenames = cell(length(scan_paths),1);
for i = 1:length(scan_paths)
    oldfilenames{i}= cellstr([split_stem{i}{1} '/structural/swc2' split_stem{i}{2}]);
    newfilenames{i}= cellstr([split_stem{i}{1} '/structural/s_8_wc2' split_stem{i}{2}]);
    try
        movefile(char(oldfilenames{i}),char(newfilenames{i}));
    catch
    end
